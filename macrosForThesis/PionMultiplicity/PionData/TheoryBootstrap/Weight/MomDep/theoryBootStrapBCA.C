...
vector<TH1D *> BootStrap(int n_throws, vector< vector<double>> data)
{
  ...
  for(int iBin = 0; iBin < data.size(); iBin++)
    {
      TH1D *h = new TH1D(Form("bin_%d",iBin),Form("bin_%d",iBin),75,0,4);
...
void theoryBootStrapBCA()
{
  ...
  */
  ...
  for(int iBin = 0; iBin < stiff_hist_vec.size(); iBin++)
    {
      cout<<stiff_hist_vec.at(iBin)->GetMean()<<endl;
      cumulative = stiff_hist_vec.at(iBin)->GetCumulative();
      cout<<"low "<<stiff_hist_vec.at(iBin)->GetMean() - cumulative->GetXaxis()->GetBinCenter(cumulative->FindFirstBinAbove(low)-1)<<endl;
      cout<<"high "<<cumulative->GetXaxis()->GetBinCenter(cumulative->FindFirstBinAbove(high)+1)-stiff_hist_vec.at(iBin)->GetMean()<<endl;
    }

	cout<<endl<<endl<<"Soft"<<endl;
  for(int iBin = 0; iBin < soft_hist_vec.size(); iBin++)
    {
      cout<<soft_hist_vec.at(iBin)->GetMean()<<endl;
      cumulative = soft_hist_vec.at(iBin)->GetCumulative();
      cout<<"low "<<soft_hist_vec.at(iBin)->GetMean() - cumulative->GetXaxis()->GetBinCenter(cumulative->FindFirstBinAbove(low)-1)<<endl;
      cout<<"high "<<cumulative->GetXaxis()->GetBinCenter(cumulative->FindFirstBinAbove(high)+1)-soft_hist_vec.at(iBin)->GetMean()<<endl;
    }
