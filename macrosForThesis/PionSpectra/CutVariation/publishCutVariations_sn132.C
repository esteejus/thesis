...
void publishCutVariations_sn132(int this_var = 3,TString filename = "blank")
{
  ...
  TFile *f = TFile::Open("cutVariaiton_singleRatio_extendMult.root");
  ...
  */
  ...
  //singleRatio[Ny - iBin][iVar]->GetYaxis()->ChangeLabel(labels.at(iVar).at(0),labels.at(iVar).at(1),labels.at(iVar).at(2),labels.at(iVar).at(3),labels.at(iVar).at(4),labels.at(iVar).at(5),labels.at(iVar).at(6));
      ...
      arrowL->Draw();
      arrowR->Draw();    
      C -> SaveAs(filename + Form("_%d.png",iVar));
