...
void plotPt108(int run = 108)
{
 ...
 vector<double> bin = {0, 20, 40, 60 , 80, 100, 120, 140, 160, 180, 200, 220, 240, 280, 320, 450}; // bestw
  ...
  for(int i = 1; i <= pim_bin->GetNbinsX(); i++)
    {
      double pim_error = pim_bin->GetBinError(i);     
      double pip_error = pip_bin->GetBinError(i);
      
      pim_bin->SetBinContent(i,pim_bin->GetBinContent(i)/pim_bin->GetBinCenter(i));
      pip_bin->SetBinContent(i,pip_bin->GetBinContent(i)/pip_bin->GetBinCenter(i));

      pim_bin->SetBinError(i,pim_error/pim_bin->GetBinCenter(i));
      pip_bin->SetBinError(i,pip_error/pip_bin->GetBinCenter(i));
