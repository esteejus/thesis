...
void theoryBootStrapBCA_108Single()
{
  ...
  loadVec(soft_data,pBUU_soft_no_DR);//idx_max = 3    
  ...
  loadVec(stiff_data,pBUU_stiff_no_DR);//idx_max
 ...
 for(int iBin = 0; iBin < xbuu_DR->GetN(); iBin++)
  ...
  TFile *out = new TFile("deltaPot_sn108.root","RECREATE");
