...
void publishDoubleRatio_samePannel()
{
...
TFile *pBUU_stiff_no_f = TFile::Open("/home/justin/Homework/pBUU/K265EM070/input_for_esym.info/pBUUStiff_r-1.root");
  ...
  */
  ...
  doubleRatio->GetXaxis()->SetTitle("T_{#scale[.6]{CM}} (MeV)");
  doubleRatio->GetXaxis()->CenterTitle();

  doubleRatio->GetYaxis()->SetRangeUser(1.7,3.5);
  ...
  int tuQMD_Soft_mc = kBlue - 4;
  int tuQMD_Soft_fc = kBlue - 4;
  int tuQMD_Soft_fs = 1;
  ...
  int tuQMD_Stiff_mc = kRed - 4;
  int tuQMD_Stiff_fc = kRed - 4;
  int tuQMD_Stiff_fs = 1;
  int tuQMD_Stiff_ms = 20;
  int tuQMD_Stiff_msz = 3;
  int tuQMD_Stiff_ls = 6;
  int tuQMD_Stiff_lw = 5;
  
  int xBUU_Soft_mc = kBlue - 7;
  int xBUU_Soft_fc = kBlue - 7;
  int xBUU_Soft_fs = 1;
  int xBUU_Soft_ms = 28;
  int xBUU_Soft_msz = 3;
  int xBUU_Soft_lw = 5;
  ...
  int xBUU_Stiff_mc = kRed - 7;
  int xBUU_Stiff_fc = kRed - 7;
  int xBUU_Stiff_fs = 1;
  ...
  int pBUUSoft_mc = kBlue - 9;
  int pBUUSoft_fc = kBlue - 9;
  ...
  int pBUUStiff_mc = kRed - 9;
  int pBUUStiff_fc = kRed - 9;
  ...
  */
  ...
  TPaveText *p_soft = new TPaveText(200,1.98,220,2.18);
  p_soft->AddText("Soft");
  p_soft->SetTextColor(kBlue-3);
  p_soft->SetFillStyle(0);
  p_soft->SetBorderSize(0);
  
  TPaveText *p_stiff = new TPaveText(180,2.35,200,2.95);
  p_stiff->AddText("Stiff");
  p_stiff->SetTextColor(kRed-3);
  p_stiff->SetFillStyle(0);
  p_stiff->SetBorderSize(0);

  TLegend *leg_soft = new TLegend(.15,.7,.4,.9);
  leg_soft->AddEntry(pBUU_soft_no_DR,"pBUU","lpe");
  leg_soft->AddEntry(xbuu_DR,"#chiBUU","lpe");
  leg_soft->AddEntry(tuQMD_05_DR,"TuQMD","lpe");
  leg_soft->AddEntry(doubleRatio,"Data","lpe");
  ...
  TLegend *leg_stiff = new TLegend(.35,.75,.6,.9);
  ...
  //  cvs_2->cd(1);
  doubleRatio->GetYaxis()->SetNdivisions(505);
