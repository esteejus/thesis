...
void publishIsoScale_pim()
{
  ...
  leg->AddEntry(pim_sn132,"{}^{132}Sn +{}^{124}Sn","lpe");

  TCanvas *cvs = style::stdcvs();
  //  cvs->SetLogy();
  //  singleRatio_sn132->GetYaxis()->SetRangeUser(.5,20);
  
  pim_sn132->GetYaxis()->SetRangeUser(1.2,2.2);
  pim_sn132->GetYaxis()->SetTitle("#pi^{-}_{132}/#pi^{-}_{108}");
  pim_sn132->GetYaxis()->CenterTitle();
  pim_sn132->GetXaxis()->SetTitle("E_{#scale[.6]{COM}} (MeV)");
  pim_sn132->GetXaxis()->CenterTitle();
  
  pim_sn132->SetLineWidth(5);
  pim_sn132->SetLineStyle(lineS132);
  pim_sn132->SetLineColor(lineC132);
  pim_sn132->SetMarkerStyle(markS132);
  pim_sn132->SetMarkerSize(markSz132);
  pim_sn132->SetMarkerColor(markC132);
  //  pim_sn132->Draw("L E1");
  //pim_sn132->DrawCopy("same hist L");
  
  pim_sn108->SetLineWidth(5);
  pim_sn108->SetLineStyle(lineS108);
  pim_sn108->SetLineColor(lineC108);
  pim_sn108->SetMarkerStyle(markS108);
  pim_sn108->SetMarkerSize(markSz108);
  pim_sn108->SetMarkerColor(markC108);
  //  pim_sn108->Draw("same E1");
  //  pim_sn108->DrawCopy("same hist L");
  leg->SetBorderSize(0);
  leg->Draw("same");
  ...
  tuQMD_sum_pim->SetFillColorAlpha(tuQMD_Soft_fc,.5);
  tuQMD_sum_pim->SetFillStyle(tuQMD_Soft_fs);
  tuQMD_sum_pim->SetMarkerColor(tuQMD_Soft_mc);
  tuQMD_sum_pim->SetMarkerSize(tuQMD_Soft_msz);
  tuQMD_sum_pim->SetMarkerStyle(tuQMD_Soft_ms);
  tuQMD_sum_pim->SetLineStyle(tuQMD_Soft_ls);
  tuQMD_sum_pim->SetLineColor(tuQMD_Soft_mc);
  tuQMD_sum_pim->SetLineWidth(tuQMD_Soft_lw);

  xbuu_sum_pim->SetFillColorAlpha(xBUU_Soft_fc,.5);
  xbuu_sum_pim->SetFillStyle(xBUU_Soft_fs);
  xbuu_sum_pim->SetMarkerColor(xBUU_Soft_mc);
  xbuu_sum_pim->SetMarkerStyle(xBUU_Soft_ms);
  xbuu_sum_pim->SetMarkerSize(xBUU_Soft_msz);
  xbuu_sum_pim->SetLineStyle(xBUU_Soft_ls);
  xbuu_sum_pim->SetLineColor(xBUU_Soft_mc);
  xbuu_sum_pim->SetLineWidth(xBUU_Soft_lw);

  urQMD_sum_pim->SetFillColorAlpha(urQMD_Soft_fc,.5);
  urQMD_sum_pim->SetFillStyle(urQMD_Soft_fs);
  urQMD_sum_pim->SetMarkerStyle(urQMD_Soft_ms);
  urQMD_sum_pim->SetMarkerColor(urQMD_Soft_mc);
  urQMD_sum_pim->SetMarkerSize(urQMD_Soft_msz);
  urQMD_sum_pim->SetLineStyle(urQMD_Soft_ls);
  urQMD_sum_pim->SetLineWidth(urQMD_Soft_lw);
  urQMD_sum_pim->SetLineColor(urQMD_Soft_mc);

  imQMD_sum_pim->SetFillColorAlpha(imQMD_Soft_fc,.5);
  imQMD_sum_pim->SetFillStyle(imQMD_Soft_fs);
  imQMD_sum_pim->SetMarkerStyle(imQMD_Soft_ms);
  imQMD_sum_pim->SetMarkerColor(imQMD_Soft_mc);
  imQMD_sum_pim->SetMarkerSize(imQMD_Soft_msz);
  imQMD_sum_pim->SetLineStyle(imQMD_Soft_ls);
  imQMD_sum_pim->SetLineWidth(imQMD_Soft_lw);
  imQMD_sum_pim->SetLineColor(imQMD_Soft_mc);

  amdJAM_sum_pim->SetFillColorAlpha(amdSoft_fc,.5);
  amdJAM_sum_pim->SetFillStyle(amdSoft_fs);
  amdJAM_sum_pim->SetMarkerStyle(amdSoft_ms);
  amdJAM_sum_pim->SetMarkerColor(amdSoft_mc);
  amdJAM_sum_pim->SetMarkerSize(amdSoft_msz);
  amdJAM_sum_pim->SetLineStyle(amdSoft_ls);
  amdJAM_sum_pim->SetLineColor(amdSoft_mc);
  amdJAM_sum_pim->SetLineWidth(amdSoft_lw);

  pBUU_sum_pim->SetFillColorAlpha(pBUUSoft_fc,.5);
  pBUU_sum_pim->SetFillStyle(pBUUSoft_fs);
  pBUU_sum_pim->SetMarkerStyle(pBUUSoft_ms);
  pBUU_sum_pim->SetMarkerColor(pBUUSoft_mc);
  pBUU_sum_pim->SetMarkerSize(pBUUSoft_msz);
  pBUU_sum_pim->SetLineStyle(pBUUSoft_ls);
  pBUU_sum_pim->SetLineColor(pBUUSoft_mc);
  pBUU_sum_pim->SetLineWidth(pBUUSoft_lw);
  ...
  leg_soft->AddEntry(tuQMD_sum_pim,"TuQMD","FL");
  leg_soft->AddEntry(urQMD_sum_pim,"urQMD","FL");
  leg_soft->AddEntry(imQMD_sum_pim,"IQMD-BNU","FL");
  leg_soft->AddEntry(xbuu_sum_pim,"#chiBUU","FL");
  leg_soft->AddEntry(amdJAM_sum_pim,"AMD + JAM","FL");
  leg_soft->AddEntry(pBUU_sum_pim,"pBUU","FL");
  ...
  leg_stiff->AddEntry(pim_sn132,"Data","lpe");

  leg_stiff->SetFillStyle(0);
  leg_stiff->SetBorderSize(0);

  pim_sn132->GetXaxis()->SetNdivisions(505);
  pim_sn132->Draw("L E1");
 
  pBUU_sum_pim->Draw("same  LE3 ");
  imQMD_sum_pim->Draw("same  LE3");
  xbuu_sum_pim->Draw("same  LE3 ");
  tuQMD_sum_pim->Draw("same  LE3");
  amdJAM_sum_pim->Draw("same  LE3 ");
  urQMD_sum_pim->Draw("same  LE3");
 
  pim_sn132->DrawCopy("same L E1");  
  leg_soft->Draw();
  leg_stiff->Draw();
