...
double ratio_fcn(double *x, double *par)
{
  ...
  double z = 57;
  double r_o = 1.25; //fm
  double T = 29; //temperature 
  
  double sigma_pip = kp_spl->Eval(ke)*pow(2*a,bp_spl->Eval(ke));
  double sigma_pim = km_spl->Eval(ke)*pow(2*a,bm_spl->Eval(ke));
 ...
 */
  ...
  vector<double> ke_paper = {36.996205208191,51.9207113626679,68.4883543306454,83.2144780955291,99.7748852316672,125.369228419132,151.58645613076,187.738480957703,233.585314477292,280.304065733512};

  vector<double> ratio_pap = {1.70408060716669,1.20369268618197,1.1530418633071,0.994685683504716,0.920036018362934,0.807940923452939,0.761746167019079,0.665961843046768,0.724626349683633,0.675151349482638};
